...
export default function IndicadorB2() {
  const hoje = new Date();
  ...
  function carregar() {
    ...
    const qs = p.toString();
    fetch(`${API}/api/indicadores/b2${qs ? "?" + qs : ""}`)
      .then((r) => r.json())
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">B2 — Tratamento Odontológico Concluído</h1>
      <p className="text-sm text-gray-500">Proporção de tratamentos odontológicos concluídos entre pessoas com primeira consulta programática — por equipe</p>
      ...
      <div className="overflow-x-auto rounded-xl shadow border border-gray-200">
        <table className="table table-zebra w-full">
          <thead className="bg-cyan-700 text-white">
            <tr><th>Unidade</th><th>Equipe</th><th>INE</th><th>1ª Consulta</th><th>Trat. Concluído</th><th>%</th><th>Classificação</th></tr>
          </thead>
          <tbody>
            {filtrados.map((r, i) => (
              <tr key={i}>
                ...
                <td>{r.pessoas_1a_consulta ?? 0}</td>
                <td className="font-bold">{r.pessoas_tratamento_concluido ?? 0}</td>
      ...
      <div className="flex gap-4 text-xs text-gray-500 bg-white p-3 rounded-xl shadow flex-wrap">
        <span className="text-green-600 font-semibold">Ótimo (&gt;75%)</span>
        <span className="text-blue-600 font-semibold">Bom (&gt;50% e ≤75%)</span>
        <span className="text-yellow-600 font-semibold">Suficiente (&gt;25% e ≤50%)</span>
        <span className="text-red-600 font-semibold">Regular (≤25%)</span>
      </div>

      <BuscaAtiva indicador="b2" label="Tratamento Concluído"
