...
export default function IndicadorB3() {
  const hoje = new Date();
  ...
  function carregar() {
    ...
    const qs = p.toString();
    fetch(`${API}/api/indicadores/b3${qs ? "?" + qs : ""}`)
      .then((r) => r.json())
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">B3 — Taxa de Exodontias Realizadas</h1>
      <p className="text-sm text-gray-500">Menor-melhor. Proporção de exodontias entre procedimentos odontológicos individuais (CD + TSB) — por equipe ESB</p>
      ...
      <div className="overflow-x-auto rounded-xl shadow border border-gray-200">
        <table className="table table-zebra w-full">
          <thead className="bg-cyan-700 text-white">
            <tr><th>Unidade</th><th>Equipe</th><th>INE</th><th>Exodontias</th><th>Prev/Cura</th><th>Total</th><th>% Exo</th><th>Classificação</th></tr>
          </thead>
          <tbody>
            {filtrados.map((r, i) => (
              <tr key={i}>
                ...
                <td className="font-bold">{r.exodontias ?? 0}</td>
                <td>{(r.total_procedimentos ?? 0) - (r.exodontias ?? 0)}</td>
                <td>{r.total_procedimentos ?? 0}</td>
                <td className="font-bold">{r.percentual ?? 0}%</td>
                <td>{r.classificacao && (
                  <span className={`badge badge-sm ${r.classificacao === "Ótimo" ? "badge-success" : r.classificacao === "Bom" ? "badge-info" : r.classificacao === "Suficiente" ? "badge-warning" : "badge-error"}`}>
                    {r.classificacao}
                  </span>
                )}</td>
              </tr>
      ...
      <div className="flex gap-4 text-xs text-gray-500 bg-white p-3 rounded-xl shadow flex-wrap">
        <span className="text-green-600 font-semibold">Ótimo (≥3% e &lt;10%)</span>
        <span className="text-blue-600 font-semibold">Bom (≥10% e &lt;12%)</span>
        <span className="text-yellow-600 font-semibold">Suficiente (≥12% e &lt;14%)</span>
        <span className="text-red-600 font-semibold">Regular (&lt;3% ou ≥14%)</span>
      </div>

      <BuscaAtiva indicador="b3" label="Exodontia"
