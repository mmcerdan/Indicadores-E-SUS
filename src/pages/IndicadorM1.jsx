...
export default function IndicadorM1() {
  const hoje = new Date();
  ...
  function carregar() {
    ...
    const qs = p.toString();
    fetch(`${API}/api/indicadores/m1${qs ? "?" + qs : ""}`)
      .then((r) => r.json())
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">Indicadores eMulti</h1>
      <p className="text-sm text-gray-500">Média de atendimentos por pessoa (eMulti) — por equipe</p>
      ...
      <div className="overflow-x-auto rounded-xl shadow border border-gray-200">
        <table className="table table-zebra w-full">
          <thead className="bg-purple-700 text-white">
            <tr><th>Unidade</th><th>Equipe</th><th>INE</th><th>Total Pessoas</th><th>Total Atend.</th><th>Média</th><th>Classificação</th></tr>
          </thead>
          <tbody>
            {filtrados.map((r, i) => (
              <tr key={i}>
                ...
                <td className="font-bold">{r.media_atend_por_pessoa}</td>
