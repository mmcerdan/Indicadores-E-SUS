...
export default function IndicadorC5() {
  const hoje = new Date();
  ...
  function carregar() {
    ...
    const qs = p.toString();
    fetch(`${API}/api/indicadores/c5${qs ? "?" + qs : ""}`)
      .then((r) => r.json())
  ...
  if (loading) return <div className="flex justify-center py-20"><span className="loading loading-spinner loading-lg text-red-600" /></div>;

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">C5 — Cuidado da Pessoa com Hipertensão</h1>
      <p className="text-sm text-gray-500">Acompanhamento de pessoas com hipertensão: 4 boas práticas (A–D, 25pts cada, total 100pts/pessoa)</p>

      <details className="bg-white p-4 rounded-xl shadow text-sm">
        <summary className="cursor-pointer font-medium text-red-700">Legenda das práticas A–D</summary>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-3">
          <div><span className="font-bold text-red-600">A</span> (25pt) — ≥1 consulta médica(o)/enfermeira(o) nos últimos 6 meses</div>
          <div><span className="font-bold text-red-600">B</span> (25pt) — ≥1 aferição de pressão arterial nos últimos 6 meses</div>
          <div><span className="font-bold text-red-600">C</span> (25pt) — ≥1 registro simultâneo de peso e altura nos últimos 12 meses</div>
          <div><span className="font-bold text-red-600">D</span> (25pt) — ≥2 visitas ACS/TACS (intervalo ≥30 dias) nos últimos 12 meses</div>
        </div>
        <div className="mt-2 text-gray-400 text-xs">Fonte: Nota Metodológica SAPS/MS 2024 · Cada pessoa pode atingir até 100 pontos</div>
      </details>
      ...
      <div className="flex gap-3 items-end flex-wrap bg-white p-4 rounded-xl shadow">
        ...
        <button className="btn btn-sm bg-red-600 text-white rounded-xl hover:bg-red-700" onClick={carregar}>Filtrar</button>
        {data?.periodo && (
        ...
        <table className="table table-zebra w-full text-xs">
          <thead className="bg-red-700 text-white">
            <tr>
              ...
              <th rowSpan={2}>INE</th>
              <th rowSpan={2}>Pessoas HAS</th>
              <th colSpan={4} className="text-center border-b border-red-600">Boas Práticas (A–D)</th>
          ...
          <tbody>
            ...
            {!filtrados.length && <tr><td colSpan={12} className="text-center text-gray-400 py-8">Nenhum registro.</td></tr>}
          </tbody>
        </table>
      </div>

      <BuscaAtiva indicador="c5" label="Hipertensão"
