...
export default function IndicadorB6() {
  const hoje = new Date();
  ...
  function carregar() {
    ...
    const qs = p.toString();
    fetch(`${API}/api/indicadores/b6${qs ? "?" + qs : ""}`)
      .then((r) => r.json())
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">B6 - Tratamento Restaurador Atraumático (TRA/ART)</h1>
      <p className="text-sm text-gray-500">Proporção de TRA/ART em relação ao total de procedimentos restauradores por equipe ESB</p>
        ...
        <table className="table table-zebra w-full">
          <thead className="bg-cyan-700 text-white">
            <tr>
              ...
              <th>Equipe</th>
              <th>INE</th>
              <th className="text-right">Restauradores</th>
              <th className="text-right">TRA/ART</th>
          ...
          <tbody>
            {filtrados.map((r, i) => (
              <tr key={i}>
                ...
                <td className="text-right">{r.total_restauradores ?? 0}</td>
                <td className="text-right">{r.tra_art ?? 0}</td>
      ...
      <div className="flex gap-4 text-xs text-gray-500 items-center flex-wrap">
        ...
        <span className="text-green-600 font-semibold">Ótimo (&gt;8%)</span>
        <span className="text-blue-600 font-semibold">Bom (&gt;6% e ≤8%)</span>
        <span className="text-yellow-600 font-semibold">Suficiente (&gt;3% e ≤6%)</span>
        <span className="text-red-600 font-semibold">Regular (≤3%)</span>
      </div>

      <BuscaAtiva indicador="b6" label="TRA/ART"
