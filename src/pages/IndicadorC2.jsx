...
export default function IndicadorC2() {
  const hoje = new Date();
  ...
  function carregar() {
    ...
    const qs = p.toString();
    fetch(`${API}/api/indicadores/c2${qs ? "?" + qs : ""}`)
      .then((r) => r.json())
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">C2 — Cuidado no Desenvolvimento Infantil</h1>
      <p className="text-sm text-gray-500">Acompanhamento de crianças até 2 anos: consultas, peso/altura, visitas e vacinação</p>
      ...
      <div className="flex gap-3 items-end flex-wrap bg-white p-4 rounded-xl shadow">
        ...
        <button className="btn btn-sm bg-teal-600 text-white rounded-xl hover:bg-teal-700" onClick={carregar}>Filtrar</button>
        {data?.periodo && (
      ...
      <div className="overflow-x-auto rounded-xl shadow border border-gray-200">
        <table className="table table-zebra w-full">
          <thead className="bg-teal-700 text-white">
            <tr><th>Unidade</th><th>Equipe</th><th>INE</th><th>Total Crianças</th><th>Soma Práticas (pts)</th><th>%</th><th>Classificação</th></tr>
          </thead>
          <tbody>
            {filtrados.map((r, i) => (
              <tr key={i}>
                ...
                <td>{r.total_criancas ?? 0}</td>
                <td>{r.soma_praticas ?? 0}</td>
      ...
      <BuscaAtiva indicador="c2" label="Desenvolvimento Infantil"
