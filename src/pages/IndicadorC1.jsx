...
export default function IndicadorC1() {
  const hoje = new Date();
  ...
  function carregar() {
    ...
    const qs = p.toString();
    fetch(`${API}/api/indicadores/c1${qs ? "?" + qs : ""}`)
      .then((r) => r.json())
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">Indicadores APS</h1>
      <p className="text-sm text-gray-500">Percentual de atendimentos por demanda programada (médicos + enfermeiros) — por equipe</p>
      ...
      <div className="overflow-x-auto rounded-xl shadow border border-gray-200">
        <table className="table table-zebra w-full">
          <thead className="bg-teal-700 text-white">
            <tr><th>Unidade</th><th>Equipe</th><th>INE</th><th>Total</th><th>Programada</th><th>%</th><th>Classificação</th></tr>
          </thead>
          <tbody>
            {filtrados.map((r, i) => (
              <tr key={i}>
                ...
                <td>{r.programada}</td>
                <td className="font-bold">{r.perc_programada ?? 0}%</td>
      ...
      <BuscaAtiva indicador="c1" label="Mais Acesso"
