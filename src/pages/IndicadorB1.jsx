...
export default function IndicadorB1() {
  const hoje = new Date();
  ...
  function carregar() {
    ...
    const qs = p.toString();
    fetch(`${API}/api/indicadores/b1${qs ? "?" + qs : ""}`)
      .then((r) => r.json())
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-800">Indicadores da Odontologia</h1>
      <p className="text-sm text-gray-500">Percentual de habitantes que realizaram 1ª consulta odontológica programática (CD) — por equipe</p>
      ...
      <div className="overflow-x-auto rounded-xl shadow border border-gray-200">
        <table className="table table-zebra w-full">
          <thead className="bg-cyan-700 text-white">
            <tr><th>Unidade</th><th>Equipe</th><th>INE</th><th>1ª Consulta</th><th>População</th><th>%</th><th>Classificação</th></tr>
          </thead>
          <tbody>
            {filtrados.map((r, i) => (
              <tr key={i}>
                ...
                <td className="font-bold">{r.pessoas_1a_consulta}</td>
                <td>{r.total_pessoas ?? 0}</td>
      ...
      <div className="flex gap-4 text-xs text-gray-500 bg-white p-3 rounded-xl shadow flex-wrap">
        <span className="text-green-600 font-semibold">Ótimo (&gt;5%)</span>
        <span className="text-blue-600 font-semibold">Bom (&gt;3% e ≤5%)</span>
        <span className="text-yellow-600 font-semibold">Suficiente (&gt;1% e ≤3%)</span>
        <span className="text-red-600 font-semibold">Regular (≤1%)</span>
      </div>

      <BuscaAtiva indicador="b1" label="1ª Consulta Odontológica"
